...
public class CachedImageView implements ViewTileLoadListener
{
    ...
    {
        requestedResolution = Math.min(Math.max(0, (long) Math.floor(targetResolution)),
    ...
    private void computeTileRequestParameters()
    {
        ...
        int maxX = (constrainedViewBoundsAtRequestedResolution.getWidth() > 0d)
                ? ceilDiv((int) constrainedViewBoundsAtRequestedResolution.getMaxX(),
                        (int) tileSizeAtRequestedResolution.getWidth())
                : 0;
        int maxY = (constrainedViewBoundsAtRequestedResolution.getHeight() > 0d)
                ...
                : 0;
        int imageWidthAtRequestedResolution = (int) Math.round(MagnitudeResolutionConverter
                .convertMagnitude(imageInformation.getSizeX().get(), 0, requestedResolution));
        this.imageTileCountXAtRequestedResolution = Math.floorDiv(imageWidthAtRequestedResolution,
                imageInformation.getTileWidth().get());
        if (this.imageTileCountXAtRequestedResolution
                * imageInformation.getTileWidth().get() != imageWidthAtRequestedResolution)
            imageTileCountXAtRequestedResolution++;
    ...
    {
        return MagnitudeResolutionConverter
                .convertDimension2D(new Dimension((int) imageBoundsAtZeroResolution.getWidth(),
                        (int) imageBoundsAtZeroResolution.getHeight()), 0d, targetResolution);
    }

    private void requestTiles()
    {
        int j, i, offset;
        for (j = tilesToRequest.y; j < tilesToRequest.getMaxY(); j++)
        {
            offset = j * imageTileCountXAtRequestedResolution;
            for (i = tilesToRequest.x; i < tilesToRequest.getMaxX(); i++)
            {
                tileCache.requestTile(requestedResolution, i, j, offset + i);
            }
        }
    }
    ...
    private Dimension2D getTileSize(Dimension tileImageSize)
    {
        if (tileSizeAtRequestedResolution.equals(tileImageSize))
            ...
            return MagnitudeResolutionConverter.convertDimension2D(tileImageSize, requestedResolution,
                    targetResolution);
    }
