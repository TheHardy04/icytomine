...
public class AnnotationInserter
{
    ...
    Set<AbstractAnnotation> activeAnnotations;
    ...
    public void insertAnnotations(Rectangle2D viewBoundsAtZeroResolution, double sequenceResolution,
            Set<AbstractAnnotation> activeAnnotations, boolean retrieveProperties) throws AnnotationInserterException
    {
    ...
    private void addAnnotationToSequence(AbstractAnnotation annotation, Sequence sequence)
            throws CytomineClientException, AnnotationInserterException
    {
        ...
        // String[] terms = annotation.getAnnotationTerms(false).stream()
        // .filter(Objects::nonNull)
        // .map(t -> t.getTerm().getName().orElse(null))
        // .filter(Objects::nonNull)
        // .toArray(String[]::new);
        // roiInView.setProperty("cytomine.terms", Arrays.toString(terms));
        if (retrieveProperties)
        {
            annotation.getProperties(false).forEach(p -> {
    ...
    private ROI2DShape createRoiInView(AbstractAnnotation annotation)
            throws CytomineClientException, AnnotationInserterException
    {
        ...
        {
            roi = createPoint((Point) geometry, annotation);
        }
        else if (geometry instanceof LineString)
        {
            roi = createLineString((LineString) geometry, annotation);
        }
        else if (geometry instanceof Polygon)
        {
            roi = createPolygon((Polygon) geometry, annotation);
        }
        else if (geometry instanceof MultiPolygon)
        {
            roi = createMultiPolygon((MultiPolygon) geometry, annotation);
        }
        else
        {
        ...
        roi.setProperty("cytomine.terms", annotation.getAssociatedTerms().stream()
                .map(t -> t.getName().orElse("Unknown")).collect(Collectors.joining(",")));
        return roi;
    }

    private ROI2DPoint createPoint(Point geometry, AbstractAnnotation annotation) throws CytomineClientException
    {
        int maxY = annotation.getImage().getSizeY().get();
        double x = MagnitudeResolutionConverter
                .convertMagnitude(geometry.getCoordinate().x - viewBoundsAtZeroResolution.getMinX(), 0d,
                        sequenceResolution);
    ...
    private ROI2DPolyLine createLineString(LineString geometry, AbstractAnnotation annotation)
            throws CytomineClientException
    {
        ...
        List<Point2D> points = IntStream.range(0, size).mapToObj(i -> {
            ...
            double x = MagnitudeResolutionConverter.convertMagnitude(
                    coordinate.x - viewBoundsAtZeroResolution.getMinX(), 0d,
                    sequenceResolution);
            double y = MagnitudeResolutionConverter
                    .convertMagnitude((maxY - coordinate.y) - viewBoundsAtZeroResolution.getMinY(), 0d,
                            sequenceResolution);
    ...
    private ROI2DShape createPolygon(Polygon geometry, AbstractAnnotation annotation) throws CytomineClientException
    {
        int maxY = annotation.getImage().getSizeY().get();
    ...
    private ROI2DPath createMultiPolygon(MultiPolygon geometry, AbstractAnnotation annotation)
    {
