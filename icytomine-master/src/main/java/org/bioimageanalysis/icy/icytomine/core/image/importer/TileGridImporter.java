...
public class TileGridImporter
{
    ...
    private List<Future<TileResult>> submitTileTasks(CompletionService<TileResult> completionService)
    {
        ...
        int imageWidthAtTargetResolution = (int) Math
                .round(MagnitudeResolutionConverter.convertMagnitude(imageInformation.getSizeX().get(), 0, resolution));
        int tileCountXAtTargetResolution = Math.floorDiv(imageWidthAtTargetResolution,
                imageInformation.getTileWidth().get());
        tileCountXAtTargetResolution = (tileCountXAtTargetResolution
                * imageInformation.getTileWidth().get() == imageWidthAtTargetResolution) ? tileCountXAtTargetResolution
                        : tileCountXAtTargetResolution + 1;
        ...
        for (int x = 0; !Thread.interrupted() && x < grid.width; x++)
        {
            ...
            {
                int tileY = grid.y + y;
                int tileIndex = tileCountXAtTargetResolution * tileY + tileX;
                Tile2DKey key = new Tile2DKey(imageInformation, resolution, tileX, tileY, tileIndex);
    ...
    {
        return () -> {
            TileImporter importer = new TileImporter(key.getImage(), (int) key.getResolution(), key.getTileIndex(),
                    key.getX(), key.getY());
