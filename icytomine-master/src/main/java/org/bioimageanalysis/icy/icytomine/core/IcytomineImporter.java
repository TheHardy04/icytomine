...
public class IcytomineImporter implements SequenceIdImporter
{
    ...
    {
        // TODO implement getOMEXMLMetaData()
        return null;
    }
            ...
            throws UnsupportedFormatException, IOException, IllegalArgumentException
    {
        ...
        // Start all tasks
        int imageTileWidthAtRequestedResolution = Math.floorDiv(imageInformation.getSizeX().get(),
                imageInformation.getTileWidth().get());
        if (imageTileWidthAtRequestedResolution * imageInformation.getTileWidth().get() != imageInformation.getSizeX()
                .get())
            imageTileWidthAtRequestedResolution++;
        final int finalTileWidth = imageTileWidthAtRequestedResolution;
        for (int i = 0, x = tileRectangle.x; i < tileRectangle.width; i++, x++)
        {
            for (int j = 0, y = tileRectangle.y; j < tileRectangle.height; j++, y++)
            {
                ...
                poolCS.submit(() -> {
                    String url = imageData.getTileUrl(resolution, tile.y * finalTileWidth + tile.x, tile.x, tile.y)
                            .get();
                    ...
                    // Draw to full image
                    retrievedGraphics.drawImage(tileBI, null,
                            (tile.x - tileRectangle.x) * getTileWidth(0) - pixelOffset.width,
