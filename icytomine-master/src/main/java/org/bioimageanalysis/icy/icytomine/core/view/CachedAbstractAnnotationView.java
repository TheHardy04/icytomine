...
public class CachedAbstractAnnotationView
{
    ...
    private Set<AbstractAnnotation> annotations; // All annotations in image
    ...
    // private Cache<AbstractAnnotation, List> pointCache;
    ...
    public CachedAbstractAnnotationView(Image imageInformation) throws CytomineException
    {
        ...
        // startPointCache();
        // retrieveCurrentUserAnnotations(true);
    ...
    // this.pointCache = cacheManager.getCache(cacheName, AbstractAnnotation.class, List.class);
    // if (pointCache == null)
    // {
    // pointCache = cacheManager.createCache(cacheName, CacheConfigurationBuilder
    // .newCacheConfigurationBuilder(AbstractAnnotation.class, List.class, ResourcePoolsBuilder.heap(100))
    // .build());
    // }
    // }

    // private void retrieveCurrentUserAnnotations(boolean forceDownload) throws CytomineClientException
    // {
    // if (forceDownload)
    // {
    ...
    // annotations = imageInformation.getUserAnnotationsWithGeometry(imageInformation.getClient().getCurrentUser(),
    // forceDownload).entrySet().stream().map(Entry::getValue).collect(Collectors.toList());
    // buildGeometricHash();
    ...
    public void addTargetAnnotations(Set<? extends AbstractAnnotation> addedAnnotations)
    {
        ...
        addAnnotationsToGeometricHash(addedAnnotations);
    }
    ...
    public void removeTargetAnnotations(Set<? extends AbstractAnnotation> removedAnnotations)
    {
        ...
        removeAnnotationsFromGeometricHash(removedAnnotations);
    }
    ...
    private void buildGeometricHash()
    {
        ...
        addAnnotationsToGeometricHash(annotations);
    }
    ...
    {
        removedAnnotations.stream().forEach(a -> {
            annotationHash.removeObject(a);
        });
    }
    ...
    private void computeViewBoundsAtZeroResolution(Point2D positionAtZeroResolution, Dimension canvasSize)
    {
        ...
        viewBoundsAtZeroResolution = new Rectangle2D.Double(positionAtZeroResolution.getX(),
                positionAtZeroResolution.getY(), canvasSizeAtZeroResolution.getWidth(),
                canvasSizeAtZeroResolution.getHeight());
    }
    ...
    private void drawAnnotations() throws CytomineClientException, AnnotationViewException, InterruptedException
    {
        try
        {
            ...
            for (AbstractAnnotation a : activeAnnotations)
            {
    ...
    private void drawAnnotation(AbstractAnnotation a, boolean selected)
            throws CytomineClientException, AnnotationViewException
    {
        Geometry geometry;
    ...
    private void drawPoint(Point point, Color color, boolean selected)
    {
        ...
        int x = (int) MagnitudeResolutionConverter.convertMagnitude(point.getX() - viewBoundsAtZeroResolution.getMinX(),
                0d,
                targetResolution);
    ...
    private void drawLineString(LineString geometry, Color color, boolean selected)
    {
        ...
        IntStream.range(0, size).forEach(i -> {
            ...
            yPoints[i] = (int) MagnitudeResolutionConverter
                    .convertMagnitude((maxY - coordinate.y) - viewBoundsAtZeroResolution.getMinY(), 0,
                            targetResolution);
        });
    ...
    public void setVisibleAnnotations(Set<? extends AbstractAnnotation> newVisibleAnnotations)
    {
        ...
        addVisibleAnnotations(newVisibleAnnotations);
    }
    ...
    {
        return Collections.unmodifiableSet(visibleAnnotations);
    }

    public Set<AbstractAnnotation> getActiveAnnotations()
    {
        return Collections.unmodifiableSet(activeAnnotations);
    }
    ...
    {
        return Collections.unmodifiableSet(selectedAnnotations);
    }
    ...
    {
        // TODO implement code for accepting annotations other than current user
        // retrieveCurrentUserAnnotations(downloadAgain);
        fillVisibleAnnotations();
    }
