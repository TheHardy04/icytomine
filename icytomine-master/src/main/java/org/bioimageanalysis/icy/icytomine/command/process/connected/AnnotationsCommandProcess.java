...
public class AnnotationsCommandProcess extends ConnectedCommandProcess<String>
{
    ...
    public String call() throws Exception
    {
        ...
        for (int p = 0; p < getArguments().length; p++)
        {
            ...
            Image img = getClient().getImageInstance(imgId);
            Map<Long, AbstractAnnotation> annots = getClient().getImageAbstractAnnotations(img);
            annotationList.append("Annotations (ID, terms, Polygon):\n");
            for (AbstractAnnotation annot : annots.values())
            {
                annotationList.append(
                        annot.getId() + " "
                                + annot.getAssociatedTerms().stream().map(t -> t.getName().orElse("Not specified"))
                                        .collect(Collectors.toSet())
                                + " " + annot.getLocation().orElse("Not specified") + "\n");
            }
        }
